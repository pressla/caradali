...
function send(packet) {
    try {
      ...
      socket.send(buf, 0, buf.length, myoptions.port, myoptions.hostname, function(err) {
        //console.log(socket.address());
        if (err)
            console.log('m2m-socket-pipe.js error sending UDP packet '+err);
      });

    } catch(err) {
      console.log('m2m-socket-pipe.js EXCEPTION socket send udp '+err)
