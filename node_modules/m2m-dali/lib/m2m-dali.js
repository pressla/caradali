...
function persistentCache(what) {
    if (what=='get') {
        try {
            fs.readFileSync('/root/m2m_persist.json', 'utf8', function(err,data) {
                if (!err) {
                    var p = JSON.parse(data);
                    persist.restart_count = p.restart_count;
                } else {
                    myApp.logger.log('error', 'error reading persist cache:'+err);
                }
            });
    ...
    if (what=='set') {
        try {
            fs.writeFile('/root/m2m_persist.json', JSON.stringify(persist), 'utf8', function(err) {
...
function initme() {
    ...
    persist.restart_count= persist.restart_count+1;
    persistentCache('set');
