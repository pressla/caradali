...
exports.initPlugin = function (m2mApp, options, next) {
    ...
    sub.on('close', function () {
        myApp.logger.log('info','UDP no more listening CLOSE EVENT FIRED' );
    });

    sub.on('error', function (err) {
        myApp.logger.log('info','UDP ERROR EVENT FIRED: '+err );
    });
...
function life () {
    ...
    myApp.logger.log('state',cversion);
	try {
...
function setParams() {
    ...
    for (i in mactable.nodes) {
        if (mactable.nodes[i].node_id == myApp.node_id) {   // for me?
            ...
            myApp.logger.log('info','Nothing to change on my parameters');
        }
    }
