...
exports.executeSeq = function (seq) {
	myPlugin.logger.log('info','Begin executeSeq');
...
function executeSeqComplete(err) {
	//serialPort.close();
	if (err)
		myPlugin.logger.log('info','executeSeq Failed');
	else
		myPlugin.logger.log('info','[executeSeqComplete]');
}
	...
	{
		{
            ...
            var frame = "\x02DALI:"+adr+cmd+',2'+"\x03";
            try {
	            serialPort.write(frame, function(err, results) {
	                if (err) {
	                    myPlugin.logger.log('info', 'Error Write 1'+frame+' to DALI serialport : '+ err);
	                }
	                myPlugin.logger.log('info','DALI.BUS:'+frame+' bytes: ' + results);
	                
	            });
            } catch (err) {
            	console.log('exception write serialport '+err)
            }
        } 
    }
}
